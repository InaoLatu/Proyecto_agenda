...
Tarea *cargar_tareas(FILE *p_fic){   //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  ...
  while(!feof(p_fic)){
    ...
    fgetc(p_fic);
    fgets(p_aux->descripcion, 51, p_fic);
    ...
    fgetc(p_fic);
    fgetc(p_fic);
    if(!feof(p_fic)){ // Comprobamos si hay mas tareas
      (p_aux->siguiente)=malloc(sizeof(Tarea));
      if(p_aux->siguiente == NULL) error_memoria();
      p_aux=p_aux->siguiente;
    }
    else p_aux->siguiente = NULL;
  }
  // Creamos la lista de struct a partir de a informacion obtenida en el fichero
  
  return p_lista;
}


char *fecha_ordenador(){  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  time_t tiempo; // Variable capaz de representar el timepo(generalmente en segundos)
  char *fechayhora=(char*)malloc(20*sizeof(char));
  if(fechayhora == NULL) error_memoria();
  struct tm *p_tiempo; // Estructura contenida en la libreria tim.h cuyos campos corresponden a l dia,mes,año,hora,minutos...
...
long diftiempo(char *info){ // Recibe como parametro la cadena de caracteres que conforman la fecha introducida
  ...
  fecha2.tm_sec = 0;
  //Completamos un struct con los datos de la fecha del ordenador 
  
  free(p); // Libero la zona de memoria del malloc de la funcion fecha_ordenador
...
int *ar_identificadores(Tarea *p_inicio){  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int *iden, i=0;
...
int *ar_dependencias_dir(char *ar){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  ...
  dep=realloc(dep, (j+1)*sizeof(int));
  if(dep == NULL) error_memoria();
  *(dep+j) = 0; // Pongo un 0 en la ultima posicion del array de enteros para marcar el final
  return dep;
}
...
int *ar_dependencias_indir(int *dep_dir, Tarea *inicio){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  ...
  while(*(dep_dir+i) != 0){ // Recorremos al array de enteros(dependencias directas) hasta el 0( que introdujimos en la funcion anterior)
    ...
    while(aux != NULL){ // Recorremos la lista de tareas hasta el final
      if(*(dep_dir+i) == aux->identificador){ // Buscamos la tarea cuyo identificador sea el mismo con el que estamos operando
	if(flag==0){
	...
	// Copiamos las dependencias en cad si ess la primera vez que realizamos esta operacion(primer identificador de una tarea con dependencias)
	else{
	  if(strcmp(aux->dependencias, "-\n") != 0) // Comprobamos si la tarea tiene dependencias
...
void caratula(int longuitud){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
...
Tarea *actualizar(Tarea *inicio){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  if(inicio == NULL) return NULL;
  Tarea *aux;
  aux=inicio;
  while(aux != NULL){
    if(diftiempo(aux->plazo) < 0 && strcmp(aux->tipo, "simple\n") != 0){
      strcpy(aux->estado, "terminada\n"); //Comprobamos si el plazo de la tarea es
    }                  //posterior y si es asi copiamos en el campo estado "terminada\n"
    aux=aux->siguiente;
  }
  return inicio;
}

int *dep_inv(int iden, Tarea *inicio){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int *inv;
  int *aux;
  int i, j=0;
  ...
  inv=realloc(inv, (j+1)*sizeof(int));  //Cuando hayamos recorrido toda la lista de tareas introducimos un 0 al final de la lista de enteros
...
int *quitar_repetidos(int *array,int numerotareas){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int flag;
  int i;
  int h;
  for(i=1;i<numerotareas;i++){ 
    flag=0;
    for(h=0;h<numerotareas;h++){ // Recorremos el array de enteros 
      if(flag==1 && array[h]==i){
	array[h]=-1;	
	continue;
      }
      ...
      if(array[h]==i){ 
	flag=1;  //Con el flag indicamos que es la primera vez que encontramos el identificador 
      }
...
void imprimir_tareas(Tarea *inicio, FILE *fic){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  while(inicio != NULL){
...
Tarea *seleccionar_tareas(Tarea *inicio, char *usuario, char *estado){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  ...
  do{
    ...
    if(strcmp(strtolower(inicio->propietario), strtolower(usuario)) == 0 && strcmp(inicio->estado, estado_aux) == 0){ // Comprobamos si la tarea pertenece al usuario y si tiene el estado seleccionado
      if(seleccionadas == NULL){     //Si es la primera tarea que cogemos reservamos memoria y tendremos el puntero seleccionadas al principio
	seleccionadas=malloc(sizeof(Tarea)); //de la lista que crearemos 
...
int nuevo_identificador(int *iden){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int j=1,i=0,flag=0;
  while(1){
    flag=0;
    i=0;
...
void error_memoria(){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
...
int *anexar_tarea(Tarea *inicio, Tarea *nueva_tarea, int *iden){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int i=0;
  ...
  *(iden+i) = nueva_tarea->identificador; // Sustituimos el 0 del final por el identificador de la tarea nueva 
  i++;
...
char *strtolower(char *str){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int i=0;
  while(str[i] != '\0'){
    str[i]=tolower(str[i]);
    i++;
  }
  return str;  //Convertimos a minusculas la cadena recibida 
}

void error_abrir(char *nombre){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
...
void error_cerrar(char *nombre){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
...
int *quitar_tarea(int *iden, int tarea){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int i=0;
  while(*(iden+i) != 0){
    if(*(iden+i) == tarea){
      *(iden+i) = -1;
...
int comprobarfecha(char *str){  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  char *p=str;
  struct tm fecha1;
  int flag;
...
int bisiesto(int year)  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
{
  int esbisiesto = 0;
...
int comprueba(int dia,int mes,int ano){   //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int correcto = 0;
  if (ano >= 100 && ano < 200 ){
    correcto = 1;		        //Comprobamos si la fehca es correcta(devolviendo un 1) o si es incorrecta (devolviendo un 0)
   }
  if (mes > 12 || mes < 0){
    correcto = 0;
    return correcto;
...
Tarea *ordenar_tareas_prioridad(Tarea *inicio){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  int importancia = 1;
  Tarea *aux, *inicio_orden=NULL, *aux_orden=NULL;
  while(importancia <= 10){
    aux=inicio;
    while(aux != NULL){
      if(aux->prioridad == importancia){
	if(inicio_orden == NULL){
	  inicio_orden=malloc(sizeof(Tarea));
	  if(inicio_orden == NULL) error_memoria();
	  *inicio_orden=*aux;
	  aux_orden=inicio_orden;
	  aux_orden->siguiente=NULL;
	}
	else{
	  aux_orden->siguiente=malloc(sizeof(Tarea));
	  if(aux_orden->siguiente == NULL) error_memoria();
	  *(aux_orden->siguiente) = *aux;
	  aux_orden=aux_orden->siguiente;
	  aux_orden->siguiente=NULL;
	}
      }
      aux=aux->siguiente;
...
Tarea *crear_tarea(char *usuario, Tarea *p_inicio, int *iden){ //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  system("clear");
  printf("\t\t\tCREAR TAREA\n\v");
  Tarea *n_tarea;
  n_tarea=malloc(sizeof(Tarea));
  if(n_tarea == NULL) error_memoria();
  // Reservamos  memoria y comprobamos  si se ha podido realizar la reserva  
  ...
  fgets(n_tarea->descripcion, 51, stdin);
  do{
  ...
  while(p_lista != NULL){
    if(strcmp(strtolower(p_lista->descripcion), strtolower(n_tarea->descripcion)) == 0 && strcmp(p_lista->propietario, usuario) == 0){ // Aviso de tarea ya existente
  ...
  //Completamos los campos del struct con los datos que gneera el ordenador automaticamente 
...
int ver_tarea(Tarea *p_lista, char *usuario){  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  system("clear");
  printf("\t\t\tVER TAREA\n\v");
  if(p_lista == NULL){
  ...
  while(p_lista != NULL){ // Recorremos la lista de tareas
	...
	if(*dep_indir_rep == 0) printf(" -\n"); // Imprimimos un guion en caso de que no dependa indirectamente de ninguna tarea
	  ...
	  int *dep_indir=quitar_repetidos(dep_indir_rep, i); // Cambio los identificadores repetidos por -1
	  i=0;
	  while(*(dep_indir+i) != 0){
	    if(*(dep_indir+i) > 0  && *(dep_indir+i) != p_lista->identificador ) // Sacamos por pantalla aquellas que son positivas(de ahi que introdujesemos -1 en los repetidos) y que no sean la misma que el identificador 
	...
	printf("\n\tTareas de la que depende la tarea:\n\t\tDirectamente: -\n\t\tIndirectamente: -\n"); // Imprimo guiones en caso de que no dependa de ninguna tarea
      }
  ...
  if(flag == 0){
    printf("\n\vAVISO: Tarea no existente.\n\vPulsar ENTER para volver al menu principal del programa."); // Aviso que la tarea no existe
    while(getchar() != '\n');
    return 0;
  }
  printf("\n\vPulsar ENTER para volver al menu principal del programa.");
...
int listar_tareas(Tarea *inicio, char *usuario){  //XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX//
  system("clear");
  ...
  do{
    scanf(" %s", &tipo);
    while(getchar() != '\n');
    if(tipo == '0') return 1; // Si se elige salir vuelve al menu principal automaticamente
    if(tipo != '1' && tipo != '2' && tipo != '3') printf("\n\v\tELECCION INCORRECTA, PULSAR 1,2,3 o 0: "); // Si se pulsa una opcion invalida se muestra este mensaje
  ...
  system("clear");
  printf("\t\t\tLISTAR TAREAS\n\v");
  printf("\n\t1) Sin ordenacion\n\t2) Por prioridad descendente\n\t0) Cancelar\n\v\t¿Criterio de ordenacion? ");
  do{
    scanf(" %c", &criterio);
    while(getchar() != '\n');
    if(criterio == '0') return 1;
    if(criterio != '1' && criterio != '2') printf("\n\vELECCION INCORRECTA, PULSAR 1,2 o 0: ");
    else break;
  }
  ...
  do{
    ...
    if(modo != '1' && modo != '2') printf("\n\v\tELECCION INCORRECTA, PULSAR 1,2 o 0: "); // Muestro el mensaje si se elige una opcion no valida
    else break; // Rompo el bucle cuandos e pulse una opcion valida
  }
  while(modo != '1' && modo != '2' && modo != '0'); // Repito el bucle mientras no se seleccione una de las opciones validas
  Tarea *sel_inicio, *sel_inicio_orden=NULL;
  switch(tipo){
  case '1':{
  ...
  if(sel_inicio == NULL){
    printf("\n\vAVISO: El usuario no tiene tareas de ese tipo o no tiene tareas.\n\v"); // Aviso de que no se encontraron tareas con las especificaciones indicadas anteriormente
    printf("Pulsar ENTER para volver al menu principal del programa.");
    while(getchar() != '\n');
    return 0;
  }
  if(criterio == '2'){
    sel_inicio_orden=ordenar_tareas_prioridad(sel_inicio);
    liberar_lista(sel_inicio);
    sel_inicio=sel_inicio_orden;
  }
  if(modo == '1'){
    system("clear");
    printf("\t\t\tLISTAR TAREAS\n\v");
  ...
  if(modo == '2'){
    ...
    if(salida[strlen(salida)-1] != 't' || salida[strlen(salida)-2] != 'x' || salida[strlen(salida)-3] != 't' || salida[strlen(salida)-4] != '.'){ // Compruebo si la extension es correcta
      liberar_lista(sel_inicio);
...
Tarea *eliminar_tarea(Tarea *inicio, int *iden, char *usuario, int *marcador){
  system("clear");
  ...
  printf("\n\vIndicar el identificador de la tarea a eliminar: ");
  int tarea;
  scanf(" %i", &tarea);
  getchar();
  Tarea *aux1=inicio;
  ...
  if(strcmp(strtolower(aux1->propietario), strtolower(usuario)) != 0){
    printf("\n\vAVISO: Operacion no permitida sobre esta tarea.\n\vPulsar ENTER para volver al menu principal del programa.");
    while(getchar() != '\n');
    return inicio;
  }
  if(*dep_inv(aux1->identificador, inicio) != 0){
    printf("\n\vAVISO: Otras tareas dependen de esta tarea.\n\vPulsar ENTER para volver al menu principal del programa.");
    while(getchar() != '\n');
    return inicio;
  }
  Tarea *aux2=inicio;
  if(inicio == aux1){ // Contemplar la posibilidad de que se libere la primera o unica tarea
    inicio=aux1->siguiente;
    free(aux1);
  }
  else{
    if(aux1->siguiente == NULL){ // Contemplar la posiblidad de que se libere la ultima tarea
      while(aux2->siguiente != aux1){
	aux2=aux2->siguiente;
      }
      aux2->siguiente = NULL;
      free(aux1);
  ...
  iden=quitar_tarea(iden, tarea);
  printf("\n\vTarea eliminada con exito.\n\vPulsar ENTER para volver al menu principal del programa.");
  while(getchar() != '\n');
  *marcador=1;
  return inicio;
}
...
int personalizar () {
  char decision,opcion1,opcion2;
  int fondo=0;
  int colorletra=37;
  int i;
  
  system("clear");
  printf("\t\t\tPERSONALIZAR\n\v\t1) Cambiar fuente y fondo\n\t0) Cancelar\n\v\tSeleccione operacion: ");
  do {
    scanf(" %c", &decision);
    while(getchar() != '\n');
    if(decision == '0') return 1;
    if(decision != '1') printf("\n\tELECCION INCORRECTA, PULSAR 1 o 0: ");
  }
  while(decision != '1');
  system("clear");
  printf("\t\t\tPERSONALIZAR\n\v\t1) Negro\n\v\t2) Rojo\n\v\t3) Verde\n\v\t4) Amarillo\n\v\t5) Azul\n\v\t6) Magenta\n\v\t7) Cian\n\v\t8) Blanco\n\v\tSeleccione el color de fuente: ");
  do{
    scanf(" %c", &opcion1);
    while(getchar() != '\n');
    switch(opcion1) {
  ...
  while(colorletra == 0);
  system("clear");
  printf("\t\t\tPERSONALIZAR\n\v\t1) Negro\n\v\t2) Rojo\n\v\t3) Verde\n\v\t4) Amarillo\n\v\t5) Azul\n\v\t6) Magenta\n\v\t7) Cian\n\v\t8) Blanco\n\v\tSeleccione el color de fondo: ");
  do{
    scanf(" %c",&opcion2);
    while(getchar() != '\n');
    switch(opcion2) {
  ...
  while(fondo == 0);
  if(fondo - 10 == colorletra){
    system("clear");
    printf("\t\t\tPERSONALIZAR\n\vAVISO: Mismo color de fuente y fondo.\n\vPulsar ENTER para volver al menu principal del programa.");
    while(getchar() != '\n');
    return 0;
  }
  printf("%c[%d;%dm",0x1B,colorletra,fondo);
  system("clear");
